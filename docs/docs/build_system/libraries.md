...
This may sound weird considering that there are thousands of C libraries out there.
But the C *language* does not specify the concept of libraries. Using libraries in C consists
of several *separate* steps:

* including a header file that is just copy-pasted into your source file, like any other
*#include*
...
cryptic *undefined reference to X* errors during linking. So it is up to the programmer to
do both steps.

## C2 library design
In C2, the compiler does have a concept of library. So using a library is more of an
...
c2c uses an environment variable named *$C2_LIBDIR* specifying a list of directories where
...
To be valid, a library directory must contain a *manifest.yaml* file and one or
...
key-values under *info* describe the generic setting for that library. For *language*
...
### Interface files
While C2 does not have header files nor an *#include* mechanism, it does have *interface*
files. These are similar to regular .c2 files, except for:

* they have the .c2i extension (for c2 interface)
* function declarations do not have a body
...
* there can be only one interface file per module
...
int fprintf(FILE* stream, const char* format @(printf_format), ...);
int printf(const char* format @(printf_format), ...);
int snprintf(char* s, size_t n, const char* format @(printf_format), ...);

// ... (stuff left out)
```
C2 accepts the C types in interface files for C libraries:
...
.c2 source files. One or more modules must be exported in the manifest so they can be used
by the user program.
...
One of the goals of C2 is tight integration with plain old C. So when building a
...
Currently c2c uses a C backend. This means that the C2 sources are transpiled into
...
and *mylib_extra.h* in *application.c*. This is no different from using any other
C library.
