...
// TODO remove (needs some cleanup below)
...
class C2Identifier {
public:
    C2Identifier(SourceLocation pkgLoc_, const std::string& pkg_,
                SourceLocation idLoc_, const std::string& id_)
        : pkgLoc(pkgLoc_)
        , pkg(pkg_)
        , idLoc(idLoc_)
        , id(id_)
...
private:
    ...
    void ParseEnumType(IdentifierInfo* id, SourceLocation& idLoc, bool is_public);
    void ParseEnumMember();
