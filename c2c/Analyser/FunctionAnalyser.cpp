...
static IdentifierExpr::RefType globalDecl2RefType(const Decl *D) {
    switch (D->getKind()) {
        case DECL_FUNC:
            return IdentifierExpr::REF_FUNC;
        case DECL_VAR:
            return IdentifierExpr::REF_VAR;
        case DECL_ENUMVALUE:
            return IdentifierExpr::REF_ENUM_CONSTANT;
        case DECL_ALIASTYPE:
        case DECL_STRUCTTYPE:
        case DECL_ENUMTYPE:
        case DECL_FUNCTIONTYPE:
        ...
        case DECL_LABEL:
            return IdentifierExpr::REF_LABEL;
    }
}
            ...
            if (!Desig->isConstant()) {
                ...
                if (!Desig->getType().isIntegerType()) {
                    ...
                    if (V.isSigned() && V.isNegative()) {
                        Diag(Desig->getLocation(), diag::err_array_designator_negative) << V.toString(10)
...
QualType FunctionAnalyser::analyseEnumMinMaxExpr(BuiltinExpr *B, bool isMin) {
    ...
    B->setValue(isMin ? Enum->getMinValue()
                      : Enum->getMaxValue());
...
QualType FunctionAnalyser::analyseMemberExpr(Expr *expr, unsigned side) {
    ...
    if (isa<ModuleType>(LType)) {
        ...
        if (!ECD) {
            ...
            ETD->fullName(buf);
            Diag(member->getLocation(), diag::err_unknown_enum_constant) << buf << member->getName();
            return QualType();
        }
        ...
        if (!S.isValid()) {
            StringBuilder buf(MAX_LEN_TYPENAME);
            LType.DiagName(buf);
            Diag(M->getLocation(), diag::err_typecheck_member_reference_struct_union) << buf << M->getSourceRange()
                                                                                      << member->getLocation();
            return QualType();
        }
...
bool FunctionAnalyser::analyseBitOffsetIndex(Expr *expr, llvm::APSInt *Result, BuiltinType *BaseType) {
    ...
    if (!T.getCanonicalType().isIntegerType()) {
        StringBuilder buf;
        T.DiagName(buf);
        Diag(expr->getLocation(), diag::err_bitoffset_index_non_int) << buf << expr->getSourceRange();
        return false;
    }
    ...
    if (Val.isSigned() && Val.isNegative()) {
        Diag(expr->getLocation(), diag::err_bitoffset_index_negative) << Val.toString(10) << expr->getSourceRange();
        return false;
    }
    if (Val.ugt(BaseType->getWidth() - 1)) {
        Diag(expr->getLocation(), diag::err_bitoffset_index_too_large) << Val.toString(10) << BaseType->getName()
                                                                       << expr->getSourceRange();
        return false;
    }
    *Result = Val;
...
QualType FunctionAnalyser::analyseBitOffsetExpr(Expr *expr, QualType BaseType, SourceLocation base) {
    ...
    if (VLvalid && VRvalid) {
        if (VR > VL) {
            Diag(B->getLocation(), diag::err_bitoffset_invalid_order) << B->getLHS()->getSourceRange()
                                                                      << B->getRHS()->getSourceRange();
            return QualType();
        }
        else {
    ...
    for (unsigned i = 0; i < numValues; i++) {
        ...
        if (DesignatedInitExpr *D = dyncast<DesignatedInitExpr>(E)) {
            ...
            if (*size != -1 && currentIndex >= *size) {
                Diag(E->getLocation(), diag::err_array_designator_too_large) << D->getIndex().toString(10)
                                                                             << (int)*size;
                return;
            }
        }
...
void FunctionAnalyser::checkEnumCases(const SwitchStmt *SS, const EnumType *ET) const {
                ...
                if (ECD) {
                    ...
                    if (index != -1) {
                        if (enumHandled[index]) {
                            Diag(cond->getLocation(), diag::err_duplicate_case) << ECD->getName()
                                                                                << cond->getSourceRange();
                        }
    ...
    if (!UnhandledNames.empty()) {
        DiagnosticBuilder DB =
            Diag(SS->getCond()->getLocation(), diag::warn_missing_case) << (int)UnhandledNames.size();
