...
template<typename T>
struct simplify_type< ::c2lang::CanQual<T>> {
  using SimpleType = const T *;

  static SimpleType getSimplifiedValue(::c2lang::CanQual<T> Val) {
...
template<typename T>
struct PointerLikeTypeTraits<c2lang::CanQual<T>> {
  static void *getAsVoidPointer(c2lang::CanQual<T> P) {
    return P.getAsOpaquePtr();
  }

  static c2lang::CanQual<T> getFromVoidPointer(void *P) {
    return c2lang::CanQual<T>::getFromOpaquePtr(P);
  }
