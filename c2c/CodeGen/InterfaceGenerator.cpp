...
InterfaceGenerator::InterfaceGenerator(const std::string& targetName_, const std::string& outputDir_, const Modules& modules_)
...
void InterfaceGenerator::write(const std::string& outputDir) {
    //FileUtils::writeFile(outputDir.c_str(), outputDir + cfilename, cbuf);
}

void InterfaceGenerator::createLibHeader(bool printCode, const std::string& outputDir) {
    EmitIncludeGuard();
    EmitIncludes();
...
void InterfaceGenerator::EmitDecl(const Decl* D, StringBuilder& output) {
    assert(D);
...
void InterfaceGenerator::forwardDecl(const Decl* D) {
    EmitForwardTypeDecl(cast<TypeDecl>(D));
}

void InterfaceGenerator::fullDecl(const Decl* D) {
    EmitTypeDecl(cast<TypeDecl>(D));
}

void InterfaceGenerator::EmitIncludeGuard() {
...
void InterfaceGenerator::EmitIncludes() {
...
void InterfaceGenerator::EmitFunctionForward(const FunctionDecl* F) {
    LOG_DECL(F)
...
void InterfaceGenerator::EmitFunctionArgs(const FunctionDecl* F, StringBuilder& output) {
    LOG_DECL(F)
    output << '(';
    int count = F->numArgs();
...
void InterfaceGenerator::EmitStructType(const StructTypeDecl* S, StringBuilder& out, unsigned indent) {
    LOG_DECL(S)
    out.indent(indent);
    out << (S->isStruct() ? "struct " : "union ");
...
void InterfaceGenerator::EmitEnumType(const EnumTypeDecl* E, StringBuilder& output) {
    LOG_DECL(E)
    output << "typedef enum {\n";
...
void InterfaceGenerator::EmitFunctionType(const FunctionTypeDecl* FTD, StringBuilder& output) {
    LOG_DECL(FTD)
...
void InterfaceGenerator::EmitArgVarDecl(const VarDecl* D, StringBuilder& output, unsigned index) {
    LOG_DECL(D)
...
void InterfaceGenerator::EmitVarDecl(const VarDecl* D, StringBuilder& output, unsigned indent) {
    LOG_DECL(D)
    output.indent(indent);
    EmitTypePreName(D->getType(), output);
    output << ' ';
...
void InterfaceGenerator::EmitFunctionProto(const FunctionDecl* F, StringBuilder& output) {
    LOG_FUNC
...
void InterfaceGenerator::EmitTypePreName(QualType type, StringBuilder& output) {
    LOG_FUNC
...
void InterfaceGenerator::EmitTypePostName(QualType type, StringBuilder& output) {
    LOG_FUNC
    if (type.isArrayType()) {
...
void InterfaceGenerator::EmitAttributes(const Decl* D, StringBuilder& output) {
